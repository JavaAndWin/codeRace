...
class GameState:
    team_id: str
    pending_llm: str | None = None  # 'reasoning', 'long_context', 'self_evolution'
    price_signal: dict[str, int] = field(default_factory=dict)
    # resource_name -> remaining days during which the resource cannot be harvested.
    ...
    # Track which day we already requested each LLM category.
...
def _ingest_llm_response(turn: Turn, state: GameState) -> None:
    # Keep the latest sandbox command result for self-evolution feedback.
...
def _front_direction(turn: Turn) -> int:
    """Return +1 if front is right (challenger), -1 if front is left (defender)."""
...
def _c_wall_layout(turn: Turn) -> tuple[Pos, ...]:
    """Return all C-shape wall positions in build/repair/upgrade priority order."""
    ...
    # Front wall: center two cells highest, then outward.
    ...
    # Back corners only (the 4 empty cells are in the middle of the back side).
...
def _is_valid_wall_pos(turn: Turn, pos: Pos) -> bool:
    if not turn.land(pos):
        return False
    # Never build on the station footprint.
    station = turn.station()
...
def _weapon_sites(turn: Turn) -> tuple[Pos, ...]:
    """Desired rocket sites: 2 back, 1 side-back."""
    xmin, ymin, xmax, ymax = _station_bounds(turn)
    front = _front_direction(turn)

    if front == 1:  # challenger, back is left
        back_x = xmin - 1
        side_back_choices = [Pos(back_x, ymin - 1), Pos(back_x, ymax + 1)]
        back_pair = [Pos(back_x, ymin), Pos(back_x, ymax)]
    else:  # defender, back is right
        back_x = xmax + 1
        side_back_choices = [Pos(back_x, ymin - 1), Pos(back_x, ymax + 1)]
        back_pair = [Pos(back_x, ymin), Pos(back_x, ymax)]

    # Pick a side-back site that is not inside the station footprint.
    side_back = None
    station = turn.station()
...
def _move_toward(
    ...
    *,
    ...
    if step is None or step in claimed:
        return None
    # Basic collision check: don't step into a robot if we care.
...
def _move_near(
    ...
    *,
    ...
    """Path to a land cell adjacent to a non-land target (mine/shop/vendor/task)."""
...
def _find_adjacent_stand(
    ...
    *,
    ...
    """Find a free neighbouring cell of target to stand in."""
    station = turn.station()
    fp = station_footprint(station.pos) if station else ()
    ...
    if prefer_front:
        ...
        candidates.sort(
            key=lambda p: (
                -(p.x * front),  # front side preferred
                distance(p, role.pos),
                p.x,
                p.y,
            )
...
def _resource_priority(turn: Turn, state: GameState) -> tuple[str, ...]:
    ...
    for res in RESOURCE_PRIORITY_DEFAULT:
        if state.harvest_ban.get(res, 0) > 0:
            # Banned resources go last, regardless of price.
            priority.append((-1000, res))
            continue
        signal = state.price_signal.get(res, 0)
        # Higher signal -> higher priority.
...
# ---------------------------------------------------------------------------
# Build / upgrade / repair decision helpers
# ---------------------------------------------------------------------------
...
def _wall_needing_repair(turn: Turn, threshold: float, min_level: int) -> Unit | None:
    """Return the most damaged front-priority wall below the HP threshold."""
    layout = _c_wall_layout(turn)
...
def _select_buy_item(turn: Turn, unit: Unit) -> str | None:
    for name in BUY_PRIORITY:
        # Keep a small stock of wall fixers, but buy vouchers first.
        if name == WALL_FIXER:
            continue
        ...
        if _can_afford(turn, name):
            return name
    # Keep emergency fixers if needed.
...
def _try_use_voucher(
    ...
    """Use an upgrade voucher from the backpack on an adjacent target if possible."""
    # Weapon upgrade vouchers.
...
def _try_repair_wall(
    ...
    if _adjacent(role.pos, wall.pos):
        ...
        return True
    # Move toward the wall so we can repair next turn.
...
def _day(
    ...
    # --- Decrement harvest bans at the start of each day -------------------
...
def _worker_day_priority(
    ...
    """Assign high-priority build/upgrade tasks to this worker.

    Priority: weapon build > wall build > weapon upgrade > wall upgrade > base upgrade.
    """
    # 1. Build weapons.
    ...
    if missing:
        ...
        if stones:
            if _try_build_wall(turn, role, claimed, commands):
                return True
        # Need stone: go mine stone.
        if _collect_resource(turn, role, claimed, commands, (STONE,), state=state):
            return True

    # 3. Weapon upgrade (use voucher on an adjacent tower).
    if _try_use_voucher(turn, role, commands):
        return True

    # 4. Wall upgrade (move to a voucher target if we have one).
    if WALL_UPGRADE_VOUCHER_1 in role.backpack or WALL_UPGRADE_VOUCHER_2 in role.backpack:
        wall = _wall_needing_upgrade(turn)
        if wall:
            if _adjacent(role.pos, wall.pos):
    ...
    # 6. Shopping: if at shop, buy missing vouchers / fixers.
    shop = turn.weapon_shop_pos()
...
def _handle_emergency_repair_prep(
    ...
    """Last 20 rounds of day: if >=4 level-2 walls, one worker buys 4 fixers and stands by walls.

    Process: sell ores -> buy 4 WallFixers -> move behind walls -> repair any reachable <60% wall.
    """
    if not turn.is_day:
        return None
    ...
    if fixers < EMERGENCY_FIXER_COUNT:
        ...
        if _adjacent(repairer.pos, shop):
            ...
            if free < need:
                # Drop anything that is not a wall fixer to free space.
    ...
    # Phase 3: while on the way to the standby spot, repair any reachable <60% wall.
    ...
    # Phase 4: stand by the walls in the safe back area.
    ...
    # Already in position; keep this worker reserved for emergency repairs.
    return repairer.unit_id
...
def _wall_standby_pos(turn: Turn) -> Pos | None:
    """Safe cell behind the C-shape opening."""
    xmin, ymin, xmax, ymax = _station_bounds(turn)
...
# ---------------------------------------------------------------------------
# Pioneer day logic: tasks / treasure / self-evolution
# ---------------------------------------------------------------------------
...
def _handle_self_evolution(
    ...
    # At task point.
    if turn.phase_task:
        # We have an active task; if we already have an LLM-generated answer, submit it.
        ...
        if answer:
            ...
            return True
        # Otherwise keep working; the executeCmd path is handled separately.
        return True
    commands[pioneer.unit_id] = accept_task_command()
    return True
...
def _execute_treasure_plan(
    ...
    # Abort on permanent failure signals from the judger.
    last_result = turn.last_summon_treasure_result
    if last_result in (3, 4):
        # 3 = wrong items, 4 = treasure already empty.
        state.treasure_plan = {}
        return False
    ...
    # Validate items: every required item must be purchasable in the weapon shop.
    ...
    if missing:
        ...
        if shop and _adjacent(pioneer.pos, shop):
            ...
            if _can_afford(turn, item):
                ...
                # Cannot afford; abort treasure plan.
                state.treasure_plan = {}
            return True
        if shop:
    ...
    # Move to the treasure location and wait until the inferred opening time.
    ...
    if not _treasure_time_ready(plan, turn):
        # We are in position but it is not time yet; just wait.
        return True

    # Summon.
    commands[pioneer.unit_id] = summon_treasure_command(loc, items)
    # Clear plan after attempt; if it failed because it is not open, the result
    # code will cause abortion next turn.
    state.treasure_plan = {}
    return True


def _treasure_time_ready(plan: dict[str, Any], turn: Turn) -> bool:
    """Check whether the inferred treasure opening time has been reached."""
    ...
    if not match:
        # Could not parse; be conservative and try anyway.
        return True
...
def _plan_llm(turn: Turn, state: GameState) -> tuple[str, str]:
    if state.pending_llm:
        # Already waiting for a response; do not issue another request this turn.
        return "", ""

    # Execute a pending sandbox command from a previous self-evolution response.
    pending_cmd = state.task_state.get("pending_cmd")
    if pending_cmd:
        state.task_state.pop("pending_cmd", None)
        return "", str(pending_cmd)

    # Priority 1: reasoning (official news).
    ...
    # Priority 2: long context (folk legends).
    ...
    # Priority 3: self-evolution task (need LLM to solve).
...
def _night(
    ...
    # Pioneer operates rockets in rotation.
    if pioneer and pioneer.health > 0:
        _pioneer_operate_weapons(turn, state, pioneer, claimed, commands)

    # Workers: one may repair walls if needed, both otherwise mine/sell safely.
    workers = list(turn.workers())
...
def _assign_night_repair(
    ...
    """Assign a single worker with a WallFixer to repair/reach a damaged wall."""
...
def _pioneer_operate_weapons(
    ...
    # Try weapons in round-robin order, preferring ones off cooldown.
    ...
    # If all on cooldown, just stay near the first tower.
    tower = towers[0]
...
def _attack_target(turn: Turn, tower: Unit) -> Pos | None:
    ...
    if not targets:
        return None
    # Prioritize high-value / dangerous robots.
...
def _rocket_targets(tower: Unit, robots: tuple[Robot, ...]) -> list[Pos]:
    """Return up to tower.level rocket target positions that maximize splash damage."""
    reach = tower.range_of_attack()
    candidates = [
...
def _worker_night(
    ...
    if _collect_resource(turn, role, claimed, commands, priority, state=state):
        return
    # Fallback: move toward back/side safe area.
    standby = _wall_standby_pos(turn)
...
def _safe_escape_step(turn: Turn, role: Unit, claimed: set[Pos]) -> Pos | None:
    """Move toward the back/side, away from robots."""
    ...
    for dx, dy in NEIGHBOURS:
        ...
        if p in blocked or p in claimed or not turn.land(p):
            continue
        # Prefer back direction and distance from robots.
